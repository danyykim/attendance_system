...
# Use the `attendance:history` instead of `attendance:logs`
...
with tab3:
    ...
    # Group by name to handle multiple check-ins or outs on the same day
    st.write("Filtered logs for the selected date:")
    st.dataframe(filtered_logs_df)
    ...
    # In case of multiple check-ins/out, we can use the first check-in and the last check-out of the day
